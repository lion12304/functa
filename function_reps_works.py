...
class LatentToModulationFr(hk.Module):
...
class LatentModulatedSiren(hk.Module):
  ...
  def __init__(self,
    ...
    self.latent_to_modulation = LatentToModulationFr(
  ...
  def apply_lora2(self, x: Array, lora_v) -> Array:
    # return lora_v
    # Compute the low-rank LoRA weight matrix: W_lora = A @ B
    lora_v = lora_v[:, None] + 0.01
    W_lora = jnp.matmul(lora_v, lora_v.T)  # Shape: (f_out, f_in)
    # Apply LoRA: LoRA(x) = W_lora @ x.T -> [batch_size, f_out]
    return jnp.matmul(x, W_lora.T)  # Transpose W_lora for correct shape
  ...
  def __call__(self, coords: Array) -> Array:
    ...
    for i in range(1, self.depth - 1):
      x_lora = self.apply_lora2(x, modulations[i]['shift'])# * 1e-3
